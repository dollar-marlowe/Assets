    ...
    if(isset($_POST["stat"])){
        ...
        $string_data="";
         if($stat=="deployed" || $stat=="all"){
            $str="SELECT  GROUP_CONCAT( `category` order by `category` SEPARATOR ', ') as category   , GROUP_CONCAT( `serial` SEPARATOR ', ') as  `serial`, brgy_id,lat,`long`, count(*) as `count` FROM deployed_assets_loc i where id=". $arr_data[0]." group by  brgy_id order by brgy_id";
