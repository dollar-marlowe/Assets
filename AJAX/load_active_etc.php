...
<?php
    ...
    if(isset($_POST["status"])){
        ...
        $db->connect();
        $status=$_POST["status"];
        $sql="SELECT id,disaster_id,date_logged,disaster,`status`
        ...
        if(  $db->is_empty($sql)=="false"){
          ...
          echo "  <table class='mini_affct broder_table_strong' >
														<tr style='border-bottom:solid 1px black;'><th style='width:0px;' colspan=4><th>ACTIVE INCIDENT/DISASTER";
                         ...
                         foreach($result as $row_d)
                                                    ...
                                                    foreach($row_d as $data){
                                                        if($col<=2){
                                                            echo "<th style='width:0px;'><input type='hidden' id='etc".(intval($counte)+intval($col))."' value='$data'>";
                                                        }
                                                        else{
                                                          echo "<th>".$data;  
                                                        }
														...
														<tr><td colspan=4>
															<div class='mini_con2' id='a_mini_con$counter'>
																<table class='mini_affct broder_table_strong' >
																<tr style='border-bottom:solid 1px black;'><th style='width:0px;'><th colspan=3 style='text-align:center'>ETC LOGS OF 'DISASTER'";
																	
                                                                    $sql2="SELECT id,date_start,`status`,reso_activation_no,file_activation
                                                                    FROM etc_disaster_view where `status`!='activating' and disaster_id=".$rowd_d["disaster_id"]." order by id";
                                                                    ...
                                                                    foreach($row_e as $d2){
                                                                        if($c2<=1){
                                                                            echo "<th ><input type='radio' name='my_etc$id_add' value='$d2'>";
                                                                        }
                                                                        else{
                                                                            echo "<th>$d2";
                                                                        }
